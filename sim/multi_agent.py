...
def agent(agent_id, all_cooked_time, all_cooked_bw, net_params_queue, exp_queue):
    ...
    with tf.Session() as sess, open(LOG_FILE + '_agent_' + str(agent_id), 'wb') as log_file:
        ...
        while True:  # experience video streaming forever
            ...
            # state[1, -1] = buffer_size / BUFFER_NORM_FACTOR  # 10 sec #in grad, b_t??
            # state[2, -1] = float(video_chunk_size) / float(delay) / M_IN_K  # kilo byte / ms #in grad, x_t
            # state[3, -1] = float(delay) / M_IN_K / BUFFER_NORM_FACTOR  # 10 sec #in grad, d_t
            # state[4, :A_DIM] = np.array(next_video_chunk_sizes) / M_IN_K / M_IN_K  # mega byte
            # state[5, -1] = np.minimum(video_chunk_remain, CHUNK_TIL_VIDEO_END_CAP) / float(CHUNK_TIL_VIDEO_END_CAP) #in grad, e_t
            ...
            state[3, -1] = float(delay) / M_IN_K / BUFFER_NORM_FACTOR  # 10 sec #in grad, d_t
            # state[4, :A_DIM] = np.array(next_video_chunk_sizes) / M_IN_K / M_IN_K  # mega byte
            state[4, -1] = np.minimum(video_chunk_remain, CHUNK_TIL_VIDEO_END_CAP) / float(CHUNK_TIL_VIDEO_END_CAP) #in grad, e_t
            state[5, -1] = net_env.z_t # z_t
            state[6, -1] = net_env.buffer #q_t
            state[7, -1] = net_env.current_time
            ...
            action_on_buffer_index = bit_rate
            print bit_rate
...
def main():
    ...
    coordinator.join()
    # central_agent(net_params_queues, exp_queues)
